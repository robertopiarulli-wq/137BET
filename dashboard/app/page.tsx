...
export default function Dashboard137Bet() {
  ...
  async function fetchPredictions() {
    let query = supabase
      ...
      .select('*')
      .order('created_at', { ascending: false }); // Mostra sempre gli ultimi inseriti
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-10 border-b border-slate-700 pb-6 gap-4">
          <div>
            ...
            <p className="text-slate-400 font-medium tracking-wide">V18.5 • Transparent Ranking Engine</p>
          </div>
          <input
            type="text"
        ...
        {predictions.length === 0 && (
          <div className="text-center py-24 border-2 border-dashed border-slate-800 rounded-3xl mt-10">
            <p className="text-slate-500 font-medium">Nessun match caricato. Avvia il bot Python per popolare la Dash!</p>
          </div>
        )}
      </div>
    </div>
